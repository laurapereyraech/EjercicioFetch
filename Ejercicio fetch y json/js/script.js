...
// Realizamos el fetch para obtener los datos del JSON
fetch(DATA_URL)
  .then(response => {
    if (!response.ok) {
  ...
  .then(data => {
    // Verificamos que la estructura de los datos es la esperada
    if (data && data.students) {
      showData(data.students); // Llamamos a showData con el array de estudiantes
    } else {
